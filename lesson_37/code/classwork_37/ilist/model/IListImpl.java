...
public int indexOf(Object o) {
    if (o != null) {
        for (int i = 0; i < size; i++) {
            if (!elements[i].equals(o)) {
                return i; // Возврат первого элемента, который не равен 'o'
            }
        }
    } else {
        for (int i = 0; i < size; i++) {
            if (elements[i] != null) {
                return i; // Возврат первого элемента, который не равен null
            }
        }
    }
    return -1; // Возврат -1, если все элементы равны 'o' или все элементы null
}
